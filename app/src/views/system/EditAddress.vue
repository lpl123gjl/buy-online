...
<div class="wrapper">
    <div class="head" :style="{opacity: opa +'%'}">
        ...
        <h4 style="font-weight: 500;">更新收货地址</h4>
    </div>
    ...
    <div class="sex" :style="{opacity: opa +'%'}">
        ...
        <input type="text" placeholder="请填写收货手机号码" v-model="EditAddress.contactTel">
    </div>

    <div class="telephone" :style="{opacity: opa +'%'}">
        <p>联系人</p>
        <input type="text" placeholder="请输入收货人姓名" v-model="EditAddress.contactName">
        <input type="radio" class="r" name="sex" value="1" v-model="EditAddress.contactSex" /><span class="new">先生</span>
        <input type="radio" class="r" name='sex' value="0" v-model="EditAddress.contactSex" /><span class="new">女士</span>
    </div>

    <div class="location" :style="{opacity: opa +'%'}">
        <p>详细地址</p>
        <input type="text" placeholder="请输入详细地址" v-model="EditAddress.address" style="width: 70vw;"></input>

    </div>

    <button @click="updataAddress" :style="{opacity: opa +'%'}">更新地址</button>
    <Footer></Footer>
</div>
</template>
...
export default {
    ...
    created() {
        ...
        this.EditAddress.accountId = this.account.accountId;
        this.daId = this.$route.query.daId;
    ...
    methods: {
        updataAddress() {
            ...
            if (this.EditAddress.contactTel == '') {
                this.$message({
                    showClose: true,
                    message: '联系人电话不能为空',
                    type: 'warning'
            ...
            if (this.EditAddress.address == '') {
                this.$message({
                    showClose: true,
                    message: '详细配送地址信息不能为空',
                    type: 'warning'
                });
                return;
            }

            let url = "/deliveryaddress/update"
            this.EditAddress.address += this.board; 
            this.$axios.post(url,this.EditAddress).then((response) => {
        ...
        loadEditAddress() {
            let url = "/deliveryaddress/info/" + this.daId;
            this.$axios.get(url).then((response) => {
                if (response.data.code == 20000) {
                    this.EditAddress = response.data.resultdata;
                } else {
                    this.$message({
                        showClose: true,
                        type: 'error',
